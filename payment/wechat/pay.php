...
if($_GPC['done'] == '1') { // 支付完成后续处理
	$log = pdo_get('core_paylog', array('plid' => $params['tid']));
	// res.err_msg将在用户支付成功后返回ok，但并不保证它绝对可靠，这里通过判断支付日志来保证支付一定成功
    // 支付日志状态只能通过异步通知来修改
	if(!empty($log) && !empty($log['status'])) {
		...
		if(!is_error($site)) {
			...
			if (method_exists($site, $method)) {
				...
				exit($site->$method($ret)); // 保证了支付一定成功，本地支付日志状态已经修改
			}
		}
	}
}
...
if (!empty($_GPC['code'])) {
	...
	if (!empty($oauth['openid'])) {
		$log['openid'] = $oauth['openid']; // 获取用户openid
	}
}

$_W['uniacid'] = $log['uniacid']; // 切换统一帐号
