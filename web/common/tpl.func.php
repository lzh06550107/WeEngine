...
/**
 * 【表单控件】: 图片上传与选择控件，图片可以批量上传，但只能选择一个图片
 * @param string $name 表单input名称
 * @param string $value 表单input值，用来显示数据库中的值
 * @param string $default 默认显示的缩略图，当没有指定$value时显示的默认图片
 * @param array $options 图片上传配置信息
 * <pre>
 * 		$options['width'] = '';
 * 		$options['height'] = '';
 * 		$options['global'] = '';// 是否上传到 global 目录（公共目录）
 *      $options['class_extra'] = '';//控件容器的class类
 *     $options['dest_dir'] // 上传目录
 * 		$options['extras'] = array( // 控件的属性
 * 			image'=> 缩略图img标签的自定义属性及属性值 ,
 * 			text'=> input 控件的自定义属性及属性值，
 *          width=> 图片的显示宽度
 * 		)
 * </pre>
 * @return string
...
function tpl_form_field_image($name, $value = '', $default = '', $options = array()) {
	...
	$options['direct'] = true;
	$options['multiple'] = false; // 只选择一个图片
		...
		<script type="text/javascript">
			function showImageDialog(elm, opts, options) {
				require(["util"], function(util){
					var btn = $(elm);
					var ipt = btn.parent().prev(); // 获取选择控件
					var val = ipt.val(); // 选择控件值
					var img = ipt.parent().next().children(); // 获取所有预览图片
					options = '.str_replace('"', '\'', json_encode($options)).';
					util.image(val, function(selectedItem){
						if(selectedItem.url){
							if(img.length > 0){
								img.get(0).src = selectedItem.url;
							}
							ipt.val(selectedItem.attachment);
							ipt.attr("filename",selectedItem.filename);
							ipt.attr("url",selectedItem.url);
						}
						if(selectedItem.media_id){
							if(img.length > 0){
								img.get(0).src = "";
							}
							ipt.val(selectedItem.media_id);
						}
					}, options);
				});
			}
 ...
 * 百度富文本编辑器
 * @param $id 控件名称
 * @param string $value 数据库回显值
 * @param array $options
 * $options['height']：编辑器高度；
 * $options['allow_upload_video']：是否允许上传视频
 * $options['dest_dir']：
 * @return string
 */
