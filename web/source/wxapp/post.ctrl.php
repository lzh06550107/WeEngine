...
if ($do == 'design_method') {
    ...
    $uniacid = intval($_GPC['uniacid']); // 添加新版本按钮会带统一帐号
    if ($choose) {
        ...
        if (empty($authurl) && !empty($_W['setting']['platform']['authstate'])) { // 授权添加小程序
            $account_platform = new WxAppPlatform();
            $authurl = $account_platform->getAuthLoginUrl();
        }
        template('wxapp/choose-type'); // 该模板中定义了设计方法
...
if ($do == 'post') {
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($uniacid)) {
            ...
            $uniacid = wxapp_account_create($account_wxapp_data); // 创建小程序
            if (is_error($uniacid)) {
                iajax(3, '添加小程序信息失败', url('wxapp/post'));
            }
        } else { // 如果已经具有统一帐号，则重复使用该统一帐号，说明一个统一帐号存在多个版本的小程序
        ...
        if (!empty($_GPC['choose']['modules'])) {
            ...
            foreach ($_GPC['choose']['modules'] as $post_module) {
                ...
                $select_modules[$module['name']] = array(
                    ...
                    'defaultentry' => $post_module['defaultentry']
                );
            }
...
if ($do == 'module_binding') {
    ...
    $modules = table('module')->with(array('bindings' => function ($query) {
        return $query->where('entry', 'cover'); // 定义模块的封面入口
