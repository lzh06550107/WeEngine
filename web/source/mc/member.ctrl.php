...
if ($do == 'save_credit_setting') {
    ...
    $credit_setting = [
        'title'=>"", // 字段名称
        'enabled'=> 0 // 是否启用，credit1和credit2字段默认启用，不能修改
    ]
    */
...
if ($do == 'register_setting') {
    ...
    if (checksubmit('submit')) {
        ...
        Array
            (
                [focusreg] => 0 // 会员注册设置，0表示自动注册；1表示会员手动注册
                [item] => random // 身份验证项，email表示邮箱；mobile表示手机；random表示混合
                [audit] => 0 // 强制验证手机号/邮箱，0表示不要；1表示需要
                [type] => password // 身份验证方式，code表示随机码；password表示固定密码；hybird表示混合
            )
        */
...
if ($do == 'credit_setting') {
    $_W['page']['title'] = '积分设置';
    $credit_setting = uni_setting_load('creditnames'); // 公众号积分别名
    $credit_setting = $credit_setting['creditnames'];

    $credit_tactics = uni_setting_load('creditbehaviors'); // 公众号积分行为设置
...
if ($do == 'group') {
    if ($_W['isajax']) {
        ...
        $credit = intval($group['credit']); // 进入该组需要积分
        $credit6 = $credit - $member['credit1']; // 比较??
    ...
    if ($_W['ispost'] && $_W['isajax']) {
        ...
        if ($_GPC['op'] == 'addaddress' || $_GPC['op'] == 'editaddress') {
            ...
            if ($_GPC['op'] == 'addaddress') {
                ...
                if (!$exist_address) {
                    $post['isdefault'] = 1; // 不存在则设置第一个地址为默认地址
                }
