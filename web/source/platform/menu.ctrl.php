...
if($do == 'display') {
	...
	$type = !empty($_GPC['type']) ? intval($_GPC['type']) : MENU_CURRENTSELF; // 选项卡类型是当前默认菜单还是历史菜单列表
	if ($type == MENU_CONDITIONAL) { // 个性菜单
...
if ($do == 'post') {
	...
	if (empty($type)) {
		if (!$_W['isajax']) {
			$update_self_menu = menu_update_currentself(); // 默认菜单
			if (is_error($update_self_menu)) {
				itoast($update_self_menu['message'], '', 'info');
			}
		}
		$type = MENU_CURRENTSELF;
		$default_menu = menu_default(); // 获取公众号当前生效的默认菜单
