...
if ($do == 'post') {
	...
	if (checksubmit('submit')) {
		...
		itoast("配置保存完成，开始文件统计。。。", url('system/scan', array('do' => 'count')), 'success'); // 启动文件统计
	}
}
...
if ($do == 'count') {
	...
	itoast("文件统计完成，进行特征函数过滤。。。", url('system/scan', array('do' => 'filter_func')), 'success'); // 特征函数过滤
}
...
if ($do == 'filter_func') {
	...
	itoast("特征函数过滤完成，进行特征代码过滤。。。", url('system/scan', array('do' => 'filter_code')), 'success'); // 特征代码过滤
}
...
if ($do == 'filter_code') {
	...
	cache_write('scan:badfile', iserializer($badfiles));
	itoast("特征代码过滤完成，进行加密文件过滤。。。", url('system/scan', array('do' => 'encode')), 'success'); // 加密代码过滤
}
