...
// 升级模块
if ($do == 'upgrade') {
    $points = ext_module_bindings(); // 获取模块入口类型
    ...
    $manifest = ext_module_manifest($module_name); // 将模块XML配置文件解析为数组
    if (empty($manifest)) {
        $cloud_prepare = cloud_prepare();
        if (is_error($cloud_prepare)) {
            iajax(1, $cloud_prepare['message']);
        }
        $module_info = cloud_m_upgradeinfo($module_name); // 获取云服务模块更新信息详情
        ...
        if (!empty($_GPC['flag']) || $upgrade_support_module) { // 从云端获取模块清单
    ...
    $check_manifest_result = manifest_check($module_name, $manifest); // 检查模块配置项
    ...
    foreach ($points as $point_name => $point_info) {
        ...
        if (is_array($bindings[$point_name]) && !empty($bindings[$point_name])) {
            foreach ($bindings[$point_name] as $entry) {
                ...
                if ($point_name == 'page' && !empty($wxapp_support)) { // 发布小程序必须有page项设置
                    $entry['url'] = $entry['do'];
                    $entry['do'] = '';
                }
    ...
    if (!empty($module_info['version']['cloud_setting'])) { // 模块配置在云端
        $module['settings'] = 2;
    } else { // 模块配置在本地
...
if ($do == 'install') {
    ...
    $recycle_module = pdo_get('modules_recycle', array('modulename' => $module_name));
    // 如果模块位于回收站，则简单删除该记录同时更新缓存即可
    ...
    if (!empty($manifest)) {
        ...
        if (!is_error($module_info)) {
            if (empty($_GPC['flag'])) { //
                ...
                define('ONLINE_MODULE', true); // 在线安装模式
                $packet = cloud_m_build($module_name);
                $manifest = ext_module_manifest_parse($packet['manifest']);
            }
        } else {
    ...
    if (!$_W['ispost'] || empty($_GPC['flag'])) {
        template('system/module-group'); // 分配模块给套餐组
        exit;
    }
    if (!empty($manifest['platform']['plugin_list'])) {
        foreach ($manifest['platform']['plugin_list'] as $plugin) {
    ...
    if (strexists($manifest['install'], '.php')) {
        if (file_exists($module_path . $manifest['install'])) {
            ...
            if (defined(ONLINE_MODULE) && ONLINE_MODULE) {
                unlink($module_path . $manifest['install']);
            }
        }
    } else {
...
if ($do == 'module_detail') {
    ...
    $module_name = trim($_GPC['name']);
    $module_info = module_fetch($module_name);
    $cloud_module = cloud_m_query(); // 获取云服务模块详情
    ...
    $module_subscribes = array(); // 收集模块订阅消息类型
    ...
    $module_ban = $_W['setting']['module_receive_ban']; // 禁止模块
...
if ($do == 'uninstall') {
    ...
    cache_build_module_subscribe_type();
    cache_build_uninstalled_module(); // 更新未安装模块列表
...
if ($do == 'installed') {
    $_W['page']['title'] = '应用列表';
    if (!empty($_GPC['system_welcome'])) { // 如果是系统首页应用
...
if ($do == 'filter') {
    ...
    $module_list = user_modules($_W['uid']);
    $upgrade_modules = module_filter_upgrade(array_keys($module_list)); // 过滤出需要更新的模块
    $modules = array();
    if (!empty($module_list) && is_array($module_list)) {
