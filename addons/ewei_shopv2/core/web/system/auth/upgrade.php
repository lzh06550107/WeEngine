...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		$resp = ihttp_request('http://cloud.meishanc.com/gateway.php/a=rr&b=check', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request('http://cloud.meishanc.com/gateway.php/a=rr&b=download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
	...
	protected function updateComplete($version, $release)
	{
		...
		$sql ="UPDATE  `ims_modules` SET  `version` =  '$version',`author` =  '共赢社区',`url` =  'https://www.meishanc.com' WHERE  `name` ='ewei_shopv2'";
		pdo_query($sql);
