...
{
	public $_data_type_config = array( "单行文本", "多行文本", "下拉框", "多选框", "14" => "单选框", "图片", "身份证号码", "日期", "日期范围", "城市", "确认文本", "时间", "时间范围", "提示文本" );
		...
		foreach( $fields as $key => $value ) 
		{
			...
			if( $data_type == 0 ) 
			{
				...
				if( !empty($data[$key]) ) 
				{
					...
					{
						case 2: $mc_data["realname"] = $data[$key];
						$m_data["realname"] = $mc_data["realname"];
						break;
						case 3: $mc_data["mobile"] = $data[$key];
						$m_data["mobile"] = $mc_data["mobile"];
						break;
									...
									if( $data_type == 9 ) 
									{
										if( $array ) 
										{
											if( is_string($memberdata[$key]) ) 
											{
												...
												list($data[$key]["province"], $data[$key]["city"], $data[$key]["area"], $data[$key]["value"]) = $areas;
											}
											else 
											{
