...
class Goods_EweiShopV2Page extends PluginWebPage 
{
	...
	public function dispose_spec($spec, $groups_goods_id, $goods_id = 0) 
	{
		...
		foreach( $spec as $k => $v ) 
		{
			$data = array( "goods_option_id" => $v["goods_option_id"], "title" => $v["name"], "uniacid" => $_W["uniacid"], "marketprice" => $v["marketprice"], "single_price" => $v["single_price"], "price" => $v["price"], "stock" => $v["stock"], "specs" => $v["specs"], "groups_goods_id" => $groups_goods_id );
			if( $goods_id ) 
			{
