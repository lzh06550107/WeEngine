		...
		public function sendMessage($openid = "", $data = array( ), $message_type = "") 
		{
			...
			if( $message_type == TM_COMMISSION_AGENT_NEW && empty($usernotice["commission_agent_new"]) ) 
			{
				...
				else 
				{
					...
					$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_agent_newtitle"]) ? $tm["commission_agent_newtitle"] : "新增下线通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
			...
			else 
			{
				if( $message_type == TM_COMMISSION_ORDER_PAY && empty($usernotice["commission_order_pay"]) ) 
				{
					...
					else 
					{
						...
						$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_order_paytitle"]) ? $tm["commission_order_paytitle"] : "下线付款通知") ), "keyword3" => array( "value" => $message ) );
				...
				else 
				{
					if( $message_type == TM_COMMISSION_ORDER_FINISH && empty($usernotice["commission_order_finish"]) ) 
					{
						...
						else 
						{
							...
							$message = str_replace("[订单金额]", $data["price"], $message);
							$message = str_replace("[商品详情]", $data["goods"], $message);
							$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_order_finishtitle"]) ? $tm["commission_order_finishtitle"] : "下线确认收货通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
					...
					else 
					{
						if( $message_type == TM_COMMISSION_APPLY && empty($usernotice["commission_apply"]) ) 
						{
							...
							else 
							{
								...
								$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_applytitle"]) ? $tm["commission_applytitle"] : "提现申请提交成功"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
						...
						else 
						{
							if( $message_type == TM_COMMISSION_CHECK && empty($usernotice["commission_check"]) ) 
							{
								...
								else 
								{
									...
									$message = str_replace("[金额]", $data["commission"], $message);
									$message = str_replace("[提现方式]", $data["type"], $message);
									$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_checktitle"]) ? $tm["commission_checktitle"] : "提现申请审核处理完成"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
							...
							else 
							{
								if( $message_type == TM_COMMISSION_PAY && empty($usernotice["commission_pay"]) ) 
								{
									...
									else 
									{
										...
										$message = str_replace("[金额]", $data["commission"], $message);
										$message = str_replace("[提现方式]", $data["type"], $message);
										$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_paytitle"]) ? $tm["commission_paytitle"] : "佣金打款通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
								...
								else 
								{
									if( $message_type == TM_COMMISSION_UPGRADE && empty($usernotice["commission_upgrade"]) ) 
									{
										...
										else 
										{
											...
											$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_upgradetitle"]) ? $tm["commission_upgradetitle"] : "分销等级升级通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
									...
									else 
									{
										if( $message_type == TM_COMMISSION_BECOME && empty($usernotice["commission_become"]) ) 
										{
											...
											else 
											{
												...
												$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_becometitle"]) ? $tm["commission_becometitle"] : "成为分销商通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
										...
										else 
										{
											if( $message_type == TM_COMMISSION_APPLYMONEY && empty($usernotice["commission_apply"]) ) 
											{
												...
												else 
												{
													...
													$message = str_replace("[金额]", $data["commission"], $message);
													$message = str_replace("[提现方式]", $data["type"], $message);
													$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_applymoneytitle"]) ? $tm["commission_applymoneytitle"] : "分销商体现通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#000000" ) );
