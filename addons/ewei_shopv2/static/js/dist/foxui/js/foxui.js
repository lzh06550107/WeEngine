...
(function () {
    ...
    FoxUI.toast = {
         ...
         * 显示提示消息
         * @param text 提示消息内容
         * @param center  是否居中显示，true或者false
         * @param duration 持续显示时间，单位毫秒，默认为1200
         */
     ...
     * @param text 确认框总内容
     * @param title 确认框标题
     * @param confirmCallback 确认按钮回调函数
     ...
     * @param text 输入框显示内容
     * @param title 输入框标题
     * @param placeholder 输入框中提示信息
     * @param callback 确定按钮回调函数
     */
     ...
     * {
            ...
            mask:'true', // 是否使用遮罩
            maskTransparent:'true', // 是否透明
            maskClick:'', // 遮罩点击回调函数
            extraClass:'', // 模态框的样式类
            openCallback:'' // 模态框打开时回调函数
     * }
     * @returns {FoxUIModal}
     * @constructor
     */
        ...
        self.show = function () {
            ...
            setTimeout(function () {
                self.container.addClass("in").transitionEnd(function () {
                    if (openCallback) {
                        openCallback(self); // 打开模态框回调函数，并传入this对象
                    }
                });
            }, 0);
        };
     ...
     * {
           title:'title' ,// 对话框标题
           text:'text', // 对话框内容
           extraClass: 'fui-dialog-confirm', // 对话框样式
            prompt: { // 提示框相关配置
                placeholder: 'placeholder', //input输入框提示
                value: 'value' // input输入框默认值
            },
            buttons: [{ // 按钮组，可传入多个
                text: FoxUI.defaults.confirmText, // 按钮文本
                extraClass:'', // 按钮的样式类
                onclick: callback // 按钮回调函数
            }]
     * }
     * @returns {FoxUIModal}
     */
    FoxUI.dialog = function (params) {
        ...
        FoxUI.dialog.modal.show(); // 显示对话框

        $('.fui-dialog-btns a', FoxUI.dialog.modal.container).each(function (index, el) {
            $(el).click(function (event) {

                FoxUI.dialog.modal.close(); // 关闭对话框

                if (params.buttons[index].onclick) { // 按钮存在回调
                    if (params.prompt) {
                        params.buttons[index].onclick(FoxUI.dialog.modal.container, FoxUI.dialog.modal.container.find(':input').val(), event);
                        return;
                    }
                    params.buttons[index].onclick(FoxUI.dialog.modal, event); // 调用回调函数
                }
            });
        });
    ...
    FoxUI.loader = {
        /**
         * 显示资源加载器
         * @param text 可以是mini或者loading或者其它
         * @param iconClass 图标
         * @param duration 持续时间
         */
