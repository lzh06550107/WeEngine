...
function template_parse($str) {
	...
	/*把{data func=”site_navs” index=”” item=”” assign=”” return=”true” limit=”10” module=”we7_demo”}进行数据调用*/
 ...
 * limit - 指定获取变量时条数。
 * return - 为false时，获取到数据后直接循环输出，为true时，获取到数据后作为变量返回。
 *
 * @return string
 */
function moduledata($params = '') {
	if (empty($params[1])) { // 获取匹配的数据
		return '';
	}
	...
	$data['multiid'] = intval($_GET['t']); // 微站id
	$data['uniacid'] = intval($_GET['i']); // 统一帐号id
	$data['acid'] = intval($_GET['j']); // 子帐号id

	if (empty($modulename) || empty($funcname)) {
		return '';
	}
	$variable = var_export($data, true); // var_export用于将数组转换成字符串表示
    // 去掉'$variable[]'外面的单引号，变为$variable[]
	$variable = preg_replace("/'(\\$[a-zA-Z_\x7f-\xff\[\]\']*?)'/", '$1', $variable);
	// 组装php代码来调用模块函数
	$php = "<?php \${$assign} = modulefunc('$modulename', '{$funcname}', {$variable}); ";
	if (empty($return)) { // 如果不返回，则直接输出
...
function site_navs($params = array()) {
	...
	$condition = array();
	if(!$cid || !$ishomepage) {
		if (!empty($params['section'])) {// 获取区域位置
			$condition['section'] = intval($params['section']);
		}
		if(empty($params['multiid'])) { // 获取微站id，没有指定这获取统一帐号默认微站
			load()->model('account');
...
function template_modulehook_parser($params = array()) {
	...
	$plugin_info = module_fetch($plugin['module']); // 获取插件信息
