...
if (!empty($type)) {
    ...
    if ($type != 'delivery') {
        if ($_GPC['mix_pay']) {
            ...
            if ($credtis[$setting['creditbehaviors']['currency']] > 0 && in_array('mix', $dos) && $credtis[$setting['creditbehaviors']['currency']] < $log['card_fee']) {
                ...
                // 混合支付生成两条支付记录，一条是用余额支付的记录；另一条是剩余需要支付的记录
    ...
    if ($type == 'wechat') { // 微信支付
        ...
        $oauth = uni_setting_load('oauth', $_W['uniacid']); // 借用授权主机
        if (!empty($oauth['host'])) {
        ...
        $proxy_pay_account = payment_proxy_pay_account(); // 获取支付公众号对象实例
