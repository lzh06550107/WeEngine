...
if ($do == 'list') {
    ...
    if (!empty($detail['linkurl'])) { // 如果存在直接链接，则重定向到该地址
    ...
    if ($_W['os'] == 'android' && $_W['container'] == 'wechat' && $_W['account']['account']) {
        $subscribeurl = "weixin://profile/{$_W['account']['account']}"; // TODO 微信关注地址
    } else {
    ...
    $detail['click'] = intval($detail['click']) + 1; // 阅读量增1
