...
$oauth = $oauth_account->getOauthInfo($code); // 根据授权码获取授权token

if (is_error($oauth) || empty($oauth['openid'])) { // 如果出现错误，则重新发起授权
...
if (intval($_W['account']['level']) == 4) {
    $fan = mc_fansinfo($oauth['openid']); // 直接通过openid获取本地粉丝信息
    if (!empty($fan)) { // 如果本地已经存在该粉丝，则直接获取用户信息登录
    ...
    } else { // 如果本地没有粉丝信息，则需要到微信服务器查询
        $accObj = WeAccount::create($_W['account']);
        $userinfo = $accObj->fansQueryInfo($oauth['openid']); // 利用token和openid直接查询粉丝详细信息
        if (!is_error($userinfo) && !empty($userinfo) && !empty($userinfo['subscribe'])) { // 已经关注粉丝
...
if (intval($_W['account']['level']) != 4) {
    if (!empty($oauth['unionid'])) {
        ...
        if (!empty($mc_oauth_fan)) { // 如果已经建立映射，则更新会话
...
$forward = urldecode($_SESSION['dest_url']); // 取出目的地址
$str = '';
if (uni_is_multi_acid()) {
    $str = "&j={$_W['acid']}";
}
...
header('Location: ' . $forward); // 跳转到目的地址
exit;