...
$setting = uni_setting($_W['uniacid'], array('uc', 'passport')); // 统一帐号默认uc和会员注册参数
$uc_setting = $setting['uc'] ? $setting['uc'] : array();
$ltype = empty($setting['passport']['type']) ? 'hybird' : $setting['passport']['type']; // 是邮箱还是手机或者混合
$audit = @intval($setting['passport']['audit']); // 注册会员是否要审核
$item = !empty($setting['passport']['item']) ? $setting['passport']['item'] : 'random'; // 会员注册需要字段
$type = trim($_GPC['type']) ? trim($_GPC['type']) : 'email';
$forward = url('mc'); // 默认跳转到会员中心

if(!empty($_GPC['forward'])) { // 登录完成后跳转链接
