...
// 通过子公众号id，获取它所属的统一帐号id，并获取统一公众号详细信息
if (!empty($id)) {
...
$_W['debug'] = intval($_GPC['debug']); // 查看触发过程
$_W['acid'] = $_W['account']['acid'];
$_W['uniacid'] = $_W['account']['uniacid'];
$_W['uniaccount'] = uni_fetch($_W['uniacid']);
$_W['account']['groupid'] = $_W['uniaccount']['groupid']; // 这是什么鬼？？
// 获取指定子帐号的二维码和头像
...
//flag==1对消息加密，并生成签名，模拟微信加密消息时使用，需要$development=0
if ($_W['isajax'] && $_W['ispost'] && $_GPC['flag'] == 1) {
    $engine->encrypt();
}
//flag==2对消息解密，并验证签名，返回解密后xml，模拟微信解密消息时使用
if ($_W['isajax'] && $_W['ispost'] && $_GPC['flag'] == 2) {
    $engine->decrypt();
}
load()->func('compat.biz'); // 兼容
$_W['isajax'] = false;
$engine->start();
...
class WeEngine
{
    ...
    {
        global $_W;
        $this->account = WeAccount::create($_W['account']); // 根据统一帐号信息，初始化子账号对象
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            $_W['modules'] = uni_modules();
            $this->modules = array_keys($_W['modules']);
            $this->modules[] = 'cover'; // 封面入口
            $this->modules[] = 'default'; // 默认回复处理类
            $this->modules[] = 'reply'; // 信息回复？？
            $this->modules = array_unique($this->modules); // 去除重复模块
        }
    }

    /**
     *  模拟对消息进行加密，并生成签名，返回加密消息结构，主要模拟微信服务器发送过来的加密信息
     */
    ...
    /**
     * 模拟对本应用返回的加密消息进行解密，并验证签名，返回解密后的信息
     */
     ...
     * 启动消息分析引擎
     *
     * analyze();// 分析处理用户转递过来的请求信息,返回经过分析的消息数据 关键字 所属模块名等信息
     * process();// 根据analyze分析出来数据里的模块名去调取对应模块的process方法
     * receive();// 根据analyze分析出来数据里的模块名去调取对应模块的receive方法，不传参默认为取消关注事件
     */
    public function start()
    {
        ...
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'get') {
            $row = array();
            $row['isconnect'] = 1; // 如果连接验证通过，则设置该子账号为已经连接状态
            pdo_update('account', $row, array('acid' => $_W['acid']));
            cache_delete("uniaccount:{$_W['uniacid']}"); // 删除缓存
        ...
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            ...
            $message = $this->account->parse($postStr); // 使用对应子账号类型实例方法来解析消息
            ...
            $hitKeyword = array();
            $response = array();
            foreach ($pars as $par) { // TODO 遍历处理器列表，没有按照规则优先级进行排序？？
                if (empty($par['module'])) {
                    continue;
                }
                $par['message'] = $message;
                ...
                if ($this->isValidResponse($response)) {
                    $hitParam = $par; // 记录命中的参数
                    if (!empty($par['keyword'])) {
                        $hitKeyword = $par['keyword']; // 记录命中的关键字
                    }
                    break; // 只能有一个模块处理消息
                }
            }
            $response_debug = $response; // 记录处理模块返回消息
            $pars_debug = $pars; // 记录符合处理该消息规则数组
            // 如果是默认模块处理器处理消息，则查找符合公众号默认回复规则
            if ($hitParam['module'] == 'default' && is_array($response) && is_array($response['params'])) {
                foreach ($response['params'] as $par) { // 遍历规则，用第一个符合的规则处理器处理
                    if (empty($par['module'])) {
                        continue;
                    }
                    $response = $this->process($par);
            ...
            $resp = $this->account->response($response); // 生成响应微信服务器的xml格式消息
            // 如果消息需要加密，则加密
            if (!empty($_GET['encrypt_type']) && $_GET['encrypt_type'] == 'aes') {
            ...
            if ($_W['debug']) {
                ...
                if (count($pars_debug) == 1) { // 表示只有默认处理器处理该消息
                    $_W['debug_data']['is_default'] = 1;
                    $_W['debug_data']['params'] = $response_debug['params'];
                } else { // 弹出当前规则
                    array_pop($pars_debug);
                    $_W['debug_data']['params'] = $pars_debug;  // 保存符合处理该消息的规则数组，不包括已经处理该消息的规则
                }
                $_W['debug_data']['hitparam'] = $hitParam; // 处理该消息的规则
            ...
            ob_start();
            echo $resp; // 返回给微信服务器消息
            ob_start();
            $this->receive($hitParam, $hitKeyword, $response); // receive存在输出吗？？
            ob_end_clean();
            exit();
        }
        // 如果出错，则记录
    ...
    {
        global $_W;
        // 取消关注或者关注事件，而用户未关注时，关注事件又分为非扫码关注和扫码关注后的事件推送
        ...
        $fans = mc_fansinfo($message['from']); // 获取粉丝信息
        ...
        if (!empty($fans)) {
            ...
            if ($message['event'] == 'unsubscribe') {
                ...
                $rec = array();
                if (empty($fans['follow'])) { // 没有关注，则重新关注
                    $rec['follow'] = 1;
     ...
     * @param $par 处理规则
     * @param $keyword 关键字
     * @param $response 处理器返回消息
     */
    private function receive($par, $keyword, $response)
    {
        global $_W;
        fastcgi_finish_request(); // 响应请求，但脚本继续执行

        $subscribe = cache_load('module_receive_enable'); // 获取消息类型与模块映射关系
        $modules = uni_modules();
        // 创建核心模块订阅器对象
        ...
        if (!empty($subscribe[$this->message['type']])) { // 遍历所有订阅该类型消息的模块
            foreach ($subscribe[$this->message['type']] as $modulename) {
                ...
                if (is_error($response) && $response['errno'] == '7') { // 如果发生错误，则再次请求
     ...
     * $param = array(
    'message' => $message, // 消息
    'module' => $keyword['module'],  //模块标识
    'rule' => $keyword['rid'], // 规则id
    'priority' => $keyword['displayorder'], // 规则优先级
    'keyword' => $keyword, // 关键字
    'reply_type' => $keyword['reply_type'] // 回复消息类型
    );
     */
    private function analyze(&$message)
    {
        ...
        if (method_exists($this, 'analyze' . $message['type'])) {
            ...
            if (!empty($temp) && is_array($temp)) {
                $params += $temp; // 数字索引是附件相等于$params[] = $temp
            }
        } else {
     ...
     * 处理关注事件，把关注事件转换为文本消息
     *
     * 1、如果是带场景值关注事件，则根据场景值获取对应关键字来生成文本消息；
     * 2、如果是普通关注事件，则根据公众号配置的首次访问自动回复关键字来生成文本消息
     *
     * @param $message 消息
     * @return array
     */
    private function analyzeSubscribe(&$message)
    {
        global $_W;
        $params = array();
        $message['type'] = 'text'; // 转换为文本消息
        $message['redirection'] = true;
        // 如果是带场景值关注事件
        if (!empty($message['scene'])) {
            $message['source'] = 'qr'; // 则是扫码关注
            $sceneid = trim($message['scene']); // 二维码的参数值
            $scene_condition = '';
            ...
            if (!empty($qr)) {
                ...
                if (!empty($qr['type']) && $qr['type'] == 'scene') {
                    $message['msgtype'] = 'text'; // 微信的文本消息类型
                }
        ...
        $setting = uni_setting($_W['uniacid'], array('welcome')); // 获取公众号首次访问自动回复
    ...
    public function analyzeText(&$message, $order = 0)
    {
        ...
        $order = intval($order);
        if (!isset($message['content'])) { // 没有设置消息的内容
            return $pars;
        }
        $condition = <<<EOF
...
EOF;
    ...
    {
        if (strtolower($message['event']) == 'subscribe') { // 这可能吗？
        ...
        if (in_array($message['event'], array('pic_photo_or_album', 'pic_weixin', 'pic_sysphoto'))) {
            ...
            $message['content'] = strval($message['eventkey']); // 事件KEY值，由开发者在创建菜单时设定
            $message['source'] = $message['event']; // 消息来源, 消息二次分析(将扫码等事件转换为 text 事件.)
        ...
        if (!empty($message['eventkey'])) {
            $message['content'] = strval($message['eventkey']);
            $message['type'] = 'text';
            $message['redirection'] = true; // 是否是重定向
    ...
    private function handler($type)
    {
        ...
        global $_W;
        $params = array();
        $setting = uni_setting($_W['uniacid'], array('default_message')); // 获取默认回复消息
    ...
    private function process($param)
    {
        ...
        if ($param['module'] == 'reply') { // 系统默认消息自动回复处理器
            $processor = WeUtility::createModuleProcessor('core');
        } else { // 模块自定义的消息自动回复处理器
        ...
        $response = $processor->respond(); // 调用模块处理器来处理消息
        if (empty($response)) {
            return false;
        }
