...
/**
 * 去除转义字符串或数组中的 \ 。去掉字符串或是数组中的转义符 (\)，例如：将字符串或数组中的 \’ \“ \ \ 转换为 ‘ ” \ 并返回。
 * @param mixed $var
 * @return mixed $var
 */
...
/**
 * 转义字符串或数组中的的HTML(转换双引号，单引号和大于，小于这几个字符)
 * @param mixed $var
 * @return mixed $var
 */
 ...
 * @param int $expire 生命周期，默认为随浏览器，单位为秒
 * @param boolean $httponly  是否允许Javascript读取，防止xss攻击，建议登录等信息开启此项
 * @return boolean
 */
...
{
    global $_W;
    if (!defined('IN_MOBILE')) { // 在后端，该token的值不会改变，这有什么用？存在安全隐患
        return substr(md5($_W['config']['setting']['authkey'] . $specialadd), 8, 8);
    } else { // 在前端
        if (!empty($_SESSION['token'])) {
            /*
             * 1、如果全部过期，则删除所有token；
             * 2、如果全部没有过期，则只会剩下最近的5个token
             * 3、如果不分过期，部分没有，则只会剩下0~5个token
            */
            $count = count($_SESSION['token']) - 5;
            asort($_SESSION['token']); // 按照生成时间先后排序
            foreach ($_SESSION['token'] as $k => $v) {// $v为token创建时间
                if (TIMESTAMP - $v > 300 || $count > 0) { // token有效期为5分钟
                    unset($_SESSION['token'][$k]);
                    $count--;
                }
            }
        }
        $key = substr(random(20), 0, 4); // 在前端，该token会改变，且有有效时间
...
/**
 * 提交来源检查，验证一下表单提交，必须与 token() 函数一起使用
 * @param string $var 表单Submit名称，如果定义button提交控件没有提供name属性，则默认为submit
 * @param boolean $allowget 是否通过检查，为true，直接验证通过；
 * @return boolean
 */
function checksubmit($var = 'submit', $allowget = false)
{
    ...
    if (defined('IN_SYS')) { // 在后端
        // 或者直接通过检查或者提交token相同且来源地址必须是本站
...
function checkcaptcha($code)
{
    ...
    $_SESSION['__code'] = ''; // 清空session
    isetcookie('__code', ''); // 清空cookie
    return $return;
}

/**
 * 获取完整数据表名，为了防止表名冲突，微擎所有数据表均有前缀，此函数就是返回加表前缀的表名
 ...
 * 根据键值对数组排序
 *
 * @param array $array 需要排序的数组，必须是二维数组
 * @param string $keys 用来排序的键名，二维中的某个键
 ...
 * @param boolean $returnNear 输入参数处理方式，为true时小于区间则返回最小值，大于区间则返回最大值，否则返回本身的值
 * @return mixed
 */
 ...
 * @param string $value，可以是字符串或者数组
 * @param int 	 $options，兼容低版本Php的 options 选项，此值为 JSON_UNESCAPED_UNICODE 时，则不转义中文
 * @return mixed
 */
 ...
 * 获取附件的HTTP绝对路径
 *
 * tomedia('images/noavatar_middle.gif');
 *
...
{
    if (empty($data) || !is_array($data) || !array_key_exists('errno', $data) || (array_key_exists('errno', $data) && $data['errno'] == 0)) { // 错误号为0，则表示成功
        return false;
    } else {// 其它错误号，则表示错误编号
        return true;
    }
 ...
 * @param string $encodingaeskey 解密的token
 ...
 * @param string $encodingaeskey 加密的token
 * @param string $appid 公众号appid
 * @return string
 */
...
/**
 *  将一个 xml 字符串转化为 xml 对象，重新封装 isimplexml_load_string 函数。解决安全问题
 * @param $string xml字符串
 * @param string $class_name 填入 SimpleXMLElement 此类或是继承于此类的类
 * @param int $options 解析参数，请见 http://php.net/manual/en/libxml.constants.php
 * @param string $ns
...
/**
 * 转化html字符串中实体数据为标签数据，修复原函数在utf8编码下被转换成黑块的坑
...
/**
 * 过滤路径中可能包含的非法字符，建议通过get或是post传递的路径皆用此函数过滤
