 ...
 * @param string $value 默认为当前日期时间
 * @param boolean $withtime 是否显示时间(时分),默认为不显示
 ...
 * 【表单控件】: 日期时间范围控件
 * @param $name 表单名称
 * @param array $value 默认为当前日期时间
 * @param bool $time 是否显示时间(时分),默认为不显示
 * @return string
 */
	...
	require(["daterangepicker"], function(){
		$(function(){
			...
			$(".daterange.daterange-time").each(function(){
				var elm = this;
				$(this).daterangepicker({
					startDate: $(elm).prev().prev().val(), // 获取开始时间
					endDate: $(elm).prev().val(), // 获取结束时间
					format: "YYYY-MM-DD HH:mm",
					timePicker: true,
					timePicker12Hour : false,
					timePickerIncrement: 1,
					minuteStep: 1
				}, function(start, end){ // 选择时间后回调函数
				    // 显示选择的日期时间范围
					$(elm).find(".date-title").html(start.toDateTimeStr() + " 至 " + end.toDateTimeStr());
					$(elm).prev().prev().val(start.toDateTimeStr()); // 设置选择的开始日期
					$(elm).prev().val(end.toDateTimeStr()); // 设置选择的结束日期
				});
			});
		});
	});
