...
class Qq extends OAuth2Client {
	...
	public function __construct($ak, $sk, $calback_url = '') {
		...
		$this->calback_url = $_W['siteroot'] . 'web/index.php'; //qq回调地址
		$this->stateParam['from'] = 'qq'; // 状态参数
	}

    /**
     * 启动qq登录地址，这里是获取code
     *
     * https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=&redirect_uri=http://www.weengine172.cn/web/index.php&state=c3RhdGU9OTlkY2EyZjgmZnJvbT1xcSZtb2RlPWxvZ2lu
     *
	...
	public function user() {
		...
		$user['password'] = '';
		$user['type'] = USER_TYPE_COMMON; // 普通类型用户
	...
	public function login() {
		...
		if (!empty($user_id)) { // 如果用户已经默认绑定qq，则直接返回
			return $user_id;
		}

		if (!empty($user_bind_info)) { // 如果用户默认绑定的不是qq，则需要查找绑定映射表
