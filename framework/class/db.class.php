...
class DB {
	...
	public function __construct($name = 'master') {
		...
		$this->cfg = $_W['config']['db'];
				unset($_W['config']['db']); // 初始化DB对象后，删除配置参数
	...
	/*
		    $this->errors[] = array (
            'sql' => ' SELECT * FROM `ims_users_bind` WHERE `uid` = :param1 AND `sid`=:param2',
            'params' =>
                array (
                    ':param1' => '1',
                    ':param2' => '2'
	...
	*/
	public function getLastSql($isGetAllSql = false) {

        $data = ['sql'=>[], 'params'=>[], 'fillSql'=>[]];

	    $callback = function ($value, $key) use( &$data) {
            if(!empty($value['params'])) {

                foreach ($value['params'] as $regx => $replace ) {
                    $data['regx'][] = '/'. $regx . '/';
                    $data['params'][] = "'". $replace. "'";
                    $data['sql'] = $value['sql'];
                }
                $data['fillSql'][] = preg_replace($data['regx'], $data['params'], $data['sql']);

            }else {
                $data['fillSql'][]= $value['sql'];
            }
        };

	    array_walk($this->errors, $callback);

        if($isGetAllSql) {
            return  print_r($data['fillSql']);
        }else {
            return print_r(end($data['fillSql']));
        }
    }
