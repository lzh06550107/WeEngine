...
class WeiXinAccount extends WeAccount
{
     ...
     * @param $uniid
     * @param bool $isOpen
     * @return array|error|mixed|string
     */
    ...
    public function changeSend($send)
    {
        ...
        $token = $this->getAccessToken(); // 全局token
        if (is_error($token)) {
            return $token;
        }
    ...
    public function getJsApiTicket()
    {
        ...
        $access_token = $this->getAccessToken(); // 全局token
        if (is_error($access_token)) {
            return $access_token;
        }
    ...
    // https://mp.weixin.qq.com/wiki?action=doc&id=mp1421141082&t=0.3860347573645413
    public function getFansStat()
    {
        ...
        $result = array();
        if (!empty($summary_response['list'])) { // 获取用户增减数据
        ...
        if (!empty($cumulate_response['list'])) { // 获取累计用户数据
