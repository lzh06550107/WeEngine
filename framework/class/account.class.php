...
abstract class WeAccount
{
     ...
     * 创建子账号具体对象实例。
     *
     * @param array $acidOrAccount 子账号id或ims_account表记录实例对象
     * @return array|PhoneappAccount|WebappAccount|WeiXinAccount|WeiXinPlatform|WxappAccount|WxAppPlatform
     */
    public static function create($acidOrAccount = array())
    {
        ...
        if (is_array($acidOrAccount) && !empty($acidOrAccount)) {
            ...
            $uniaccount = table('account')->getUniAccountByAcid($acidOrAccount); // 根据acid获取主账号与子账号映射
        }
        if (is_error($uniaccount) || empty($uniaccount)) {
            $uniaccount = $_W['account'];
        }

        if (!empty($uniaccount) && isset($uniaccount['type'])) { // 根据子账号类型加载对应的类文件，来初始化该对象
     ...
     * @return PhoneappAccount|WebappAccount|WeiXinAccount|WeiXinPlatform|WxappAccount|WxAppPlatform
     */
    ...
    static public function includes($uniaccount)
    {
        ...
        $account_obj->account = $account_obj->fetchAccountInfo(); // 获取当前子账号信息
    ...
    public function parse($message)
    {
        global $_W;
        if (!empty($message)) {
            $message = xml2array($message); // xml结构消息转换为数组
            $packet = iarray_change_key_case($message, CASE_LOWER); // 键值变小写
            $packet['from'] = $message['FromUserName']; // 发送方帐号（一个OpenID）
            $packet['to'] = $message['ToUserName']; // 开发者微信号
            $packet['time'] = $message['CreateTime']; // 消息创建时间 （整型）
            $packet['type'] = $message['MsgType']; // 消息类型默认为微信消息类型，下面可能会修改
            $packet['event'] = $message['Event']; // 如果是事件消息，该字段指定事件类型
            switch ($packet['type']) { // 消息类型所特有的字段
                case 'text':
            ...
            /**
             * 消息类型同公众平台官方不同之处在于将 event 类型拆分开为独立的消息类型
             * 1、普通subscribe事件和用户未关注时扫码进行关注后的事件推送，消息类型修改为subscribe；
             * 2、用户已关注时的扫码事件推送，消息类型修改为qr;
             * 3、unsubscribe 事件，则消息类型修改为unsubscribe;
             * 4、LOCATION 事件，则消息类型修改为trace;
             * 5、pic_photo_or_album、pic_weixin、pic_sysphoto，消息类型还是event;
             * 6、card_pass_check、card_not_pass_check、user_get_card、user_del_card、user_consume_card、poi_check_notify事件，则消息类型修改为coupon;
             */
            switch ($packet['event']) { // 事件类型所特有的字段
                case 'subscribe':
                    $packet['type'] = 'subscribe'; // 这里没有加break;
                case 'SCAN':
                    if ($packet['event'] == 'SCAN') { // 扫码被重新定义事件类型为qr
                        $packet['type'] = 'qr';
                    }
                    // 对于普通关注或者用户未关注时扫码进行关注后的事件推送或者用户已关注时的扫码事件推送都会执行
                    // 如果是扫码事件推送，则获取二维码的ticket，可用来换取二维码图片和二维码的参数值
                    if (!empty($packet['eventkey'])) { // 对于订阅和扫码，则获取qrscene_为前缀，后面为二维码的参数值
                ...
                case 'unsubscribe':
                    ...
                    break;
                case 'LOCATION':
                    $packet['type'] = 'trace'; // 位置事件被重新定义为trace事件类型
                ...
                case 'pic_sysphoto': // 弹出系统拍照发图的事件推送
                    $packet['sendpicsinfo']['piclist'] = array(); // 图片列表
                    $packet['sendpicsinfo']['count'] = $message['SendPicsInfo']['Count']; // 发送的图片数量
    ...
    public function response($packet)
    {
        ...
        if (empty($packet['FuncFlag'])) { // 将消息标注星标（收藏），可将该信息永久保存。
            $packet['FuncFlag'] = 0;
        } else {
            $packet['FuncFlag'] = 1;
        }
...
class WeUtility
{
    ...
    public static function createModule($name)
    {
        ...
        if (!class_exists($classname)) {
            $file = IA_ROOT . "/addons/{$name}/module.php"; // 获取模块定义文件
            if (!is_file($file)) { // 框架内置模块
        ...
        if (!empty($GLOBALS['_' . chr('180') . chr('181') . chr('182')])) {
            ...
            require_once $codefile; // 加载模块类
            restore_include_path(); // 恢复包含路径
        }
        if (!class_exists($classname)) {
            trigger_error('Module Definition Class Not Found', E_USER_WARNING);
            return null;
        }
        $o = new $classname(); // 初始化模块对象实例
        $o->uniacid = $o->weid = $_W['uniacid']; // 模块所属的统一帐号
        $o->modulename = $name; // 所属模块标识
        $o->module = module_fetch($name); // 获取模块信息
        $o->__define = $file; // 模块类定义文件路径
        self::defineConst($o); // 定义模块根路径和访问路径
    ...
    public static function createModuleProcessor($name)
    {
        ...
        $o = new $classname(); // 初始化模块处理器
        $o->uniacid = $o->weid = $_W['uniacid']; // 模块所属统一帐号
        $o->modulename = $name; // 所属模块标识
        $o->module = module_fetch($name); // 模块信息
        $o->__define = $file; // 模块处理器类定义文件路径
        self::defineConst($o); // 定义模块根路径和访问路径
    ...
    public static function createModuleSite($name)
    {
        ...
        $o->__define = $file;
        if (!empty($o->module['plugin'])) { // 如果该模块存在插件，则获取它的插件列表
            $o->plugin_list = module_get_plugin_list($o->module['name']);
        }
        self::defineConst($o);
        $o->inMobile = defined('IN_MOBILE'); // 是否在手机端
    ...
    {
        global $_W;
        if ($_W['setting']['copyright']['log_status'] != 1) { // 需要到后台站点开启记录日志
            return false;
        }
...
abstract class WeBase
{
    ...
    {
        global $_W;
        $pars = array('module' => $this->modulename, 'uniacid' => $_W['uniacid']); // 查询条件
        $row = array(); // 更新数据
        $row['settings'] = iserializer($settings);
        cache_build_module_info($this->modulename); // 删除指定模块缓存信息
        if (pdo_fetchcolumn("SELECT module FROM " . tablename('uni_account_modules') . " WHERE module = :module AND uniacid = :uniacid", array(':module' => $this->modulename, ':uniacid' => $_W['uniacid']))) { // 如果存在，则更新
    ...
    protected function template($filename)
    {
        ...
        if (defined('IN_SYS')) { // 如果是后台页面
            //1、首先从系统后台指定主题目录中指定模块目录中开始查找
            $source = IA_ROOT . "/web/themes/{$_W['template']}/{$name}/{$filename}.html";
            $compile = IA_ROOT . "/data/tpl/web/{$_W['template']}/{$name}/{$filename}.tpl.php";
            // 2、然后是系统后台默认主题目录中指定模块目录中查找
            if (!is_file($source)) {
                $source = IA_ROOT . "/web/themes/default/{$name}/{$filename}.html";
            }
            // 3、然后是模块模板目录查找
            if (!is_file($source)) { // 模块中的模板
        ...
        // 如果是开发环境或者没有该编译文件或者编译文件过期了，则重新编译
    ...
    /**
     * 保存一个流数据到本地
     * @param string $file_string 文件内容
     * @param string $type 要保存的文件扩展名
     * @param  string $auto  文件名称
     * @return 保存的文件路径
     */
    ...
    protected function getFunctionFile($name)
    {
        ...
        if ($module_type == 'site') { // 如果是WeModuleSite类
    ...
    /**
     * 对于未定义的函数，则引入该函数名称对应的文件。
     *
     * 作用是根据函数调用名称来引入定义代码片段的php文件(注意，文件里面定义的不一定是函数，还有其它代码)
     *
     * @param $name
     * @param $param 该传入的参数可以在引入文件中直接使用。
     * @return bool
     */
    public function __call($name, $param)
    {
        ...
        if (file_exists($file)) {
            require $file; // 对于包含文件内定义的函数和类则是全局作用域，而对于其他的代码，则是函数作用域
            exit;
        }
...
abstract class WeModuleProcessor extends WeBase
{
    ...
    public function __construct()
    {
        ...
        if (!empty($_W['openid'])) {
            load()->model('mc');
            $_W['member'] = mc_fetch($_W['openid']); // 获取会员的信息
        }
    }
     ...
     * @return boolean 成功启动上下文返回true, 如果当前已经在上下文环境中也会返回true
     */
    ...
    protected function buildSiteUrl($url)
    {
        ...
        // 判断该统一账号是否有多个子帐号，则需要使用i来表示统一账号，j来表示子账号
        ...
        if ($_W['account']['level'] == ACCOUNT_SERVICE_VERIFY) { // 服务号-认证
    ...
    protected function extend_W()
    {
        ...
        if (!empty($_W['openid'])) {
            load()->model('mc');
            $_W['member'] = mc_fetch($_W['openid']); // 获取用户信息
        }
        if (empty($_W['member'])) {
            $_W['member'] = array();
        }
        ...
        if (!empty($_W['acid'])) {
            ...
            if (empty($_W['uniaccount'])) {
                $_W['uniaccount'] = uni_fetch($_W['uniacid']); // 获取统一帐号信息
            }
            if (empty($_W['account'])) { // 获取子帐号信息
...
abstract class WeModuleSite extends WeBase
{
    ...
    /**
     * 对于不存在的函数，则是引入该函数名称的文件
     * @param $name
     * @param 该传入的参数可以在引入文件中直接使用。 $arguments
     * @return bool|null
     */
    ...
    public function __get($name)
    {
        if ($name == 'module') {
            if (!empty($this->module)) {
                ...
                return getglobal('current_module'); // 获取当前模块信息
            }
        }
    }

    /**
     * 调用系统的支付功能, 只能在 Mobile 端调用。构造支付参数，然后进入支付中心界面
     * @param array $params
    ...
    protected function pay($params = array(), $mine = array())
    {
        ...
        $setting = uni_setting($_W['uniacid'], array('payment', 'creditbehaviors')); // 公众号积分行为设置
        ...
        foreach ($pay as &$value) {
            $value['switch'] = $value['pay_switch']; // 切换到付款操作
        }
        unset($value);
        if (empty($_W['member']['uid'])) {
            $pay['credit']['switch'] = false;
        }
        if ($params['module'] == 'paycenter') { // paycenter模块不支持货到付款和汇款支付
            $pay['delivery']['switch'] = false;
            $pay['line']['switch'] = false;
        }
        if (!empty($pay['credit']['switch'])) { // 如果支持余额支付
...
{
    /**
     * 如果调用函数找不到，则加载模块cron目录中的对应名称的文件
     * @param $name
     * @param 该传入的参数可以在引入文件中直接使用。 $arguments
     * @return array|bool
     */
