...
function module_save_group_package($package) {
	...
	if (user_is_vice_founder()) { // 如果是副创始人，则当前用户作为套餐组拥有者
	...
	if (!empty($package['id'])) { // 如果存在套餐组id，则
...
function module_fetch($name) {
	...
	if (empty($module)) {
		...
		if ($module_info['app_support'] != MODULE_SUPPORT_ACCOUNT && $module_info['wxapp_support'] != MODULE_SUPPORT_WXAPP && $module_info['webapp_support'] != MODULE_SUPPORT_WEBAPP && $module_info['welcome_support'] != MODULE_SUPPORT_SYSTEMWELCOME && $module_info['phoneapp_support'] != MODULE_SUPPORT_PHONEAPP) {
 ...
 * 获取未安装模块数量
 * @param $status 模块状态
 * @param bool $cache 是否从缓存获取还是云端获取
 * @param string $module_type 模块类型是公众号、小程序、PC、APP、系统首页
 * @return array
 */
function module_get_all_unistalled($status, $cache = true, $module_type = '')  {
	...
	if (!is_array($uninstall_modules) || empty($uninstall_modules['modules'][$status][$account_type]) || intval($uninstall_modules['cloud_m_count']) !== intval($cloud_m_count) || is_error($get_cloud_m_count)) {
		$uninstall_modules = cache_build_uninstalled_module(); // 触发重新构建未安装模块缓存
	}
...
function module_permission_fetch($name) {
	...
	unset($entries);
	if(!empty($module['permissions'])) { // 模块自定义权限标识
...
function module_uninstall($module_name, $is_clean_rule = false) {
	...
	ext_module_clean($module_name, $is_clean_rule); // 删除模块信息
...
function module_upgrade_new($type = 'account') {
	if ($type == 'wxapp') {
        ...
        $module_list = user_module_by_account_type('webapp');
    } else if($type == 'phoneapp') {
        $module_list = user_module_by_account_type('phoneapp');
    }
...
function module_link_uniacid_fetch($uid, $module_name) {
	...
	foreach ($accounts_list as $key => $account_value) { // TODO 只有公众号和小程序，其它类型统一帐号没有
...
function module_clerk_info($module_name) {
	...
	if (!empty($user_permissions)) {
		foreach ($user_permissions as $key => $value) { // $key为uid
