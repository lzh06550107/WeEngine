...
function user_single($user_or_uid)
{
    ...
    if (!empty($record['owner_uid'])) { // 如果用户具有上级副创始人，则获取该创始人的 名称
        $record['vice_founder_name'] = pdo_getcolumn('users', array('uid' => $record['owner_uid']), 'username');
    }
    if ($record['type'] == ACCOUNT_OPERATE_CLERK) { // 如果该用户类型为店员，则
...
function user_modules($uid)
{
    ...
    if (empty($modules)) {
        $user_info = user_single(array('uid' => $uid)); // 获取用户所有相关信息

        // 获取所有系统内置模块
        $system_modules = pdo_getall('modules', array('issystem' => 1), array('name'), 'name');
        if (empty($uid) || user_is_founder($uid) && !user_is_vice_founder($uid)) { // 主创始人则获取所有模块
            $module_list = pdo_getall('modules', array(), array('name'), 'name', array('mid DESC'));
        } elseif (!empty($user_info) && $user_info['type'] == ACCOUNT_OPERATE_CLERK) {// 如果用户是店员，则
            ...
            $module_list = array($clerk_module['type'] => $clerk_module['type']); //店员可用的模块
        } elseif (!empty($user_info) && empty($user_info['groupid'])) { // 如果用户不属于任何用于组
            $module_list = $system_modules;
        } else { // 如果用户属于某个用户组
            // 如果用户组是副创始人组
            if ($user_info['founder_groupid'] == ACCOUNT_MANAGE_GROUP_VICE_FOUNDER) { // 如果用户是副创始人
            ...
            if (!empty($packageids) && in_array('-1', $packageids)) { // 获取所有模块
                $module_list = pdo_getall('modules', array(), array('name'), 'name', array('mid DESC'));
            } else {// 获取用户组所有套餐组详情
                ...
                if (!empty($package_group)) {
                    ...
                    foreach ($package_group as $row) {
                        if (!empty($row['modules'])) { // 获取每个套餐中的模块标识组
    ...
    if (!empty($modules)) {
        foreach ($modules as $module) {
            $module_info = module_fetch($module); // 获取模块详细信息
...
function user_login_forward($forward = '')
{
    ...
    if (!empty($forward)) { // 如果指定跳转链接，则使用，否则使用下面的默认链接
    ...
    $url = user_after_login_link(); // 其它角色登录之后默认跳转链接
...
function user_module_by_account_type($type)
{
    ...
    if (!empty($module_list)) {
        foreach ($module_list as $key => &$module) {
            ...
            if ($module['phone_support'] != MODULE_NOSUPPORT_PHONEAPP && $type == 'phoneapp') {
                unset($module_list[$key]);
            }
        }
        unset($module);
...
function user_save_group($group_info)
{
    ...
    if (user_is_vice_founder()) { // 如果用户是副创始人，则创建组设置的个数不能超过该副创始人组设置的个数
...
function user_info_save($user, $is_founder_group = false)
{
    ...
    if ($is_founder_group) { // 如果是副创始人，则获取副创始人组信息
        $group = user_founder_group_detail_info($user['groupid']);
    } else { // 如果是非副创始人，则获取用户组信息
    ...
    $timelimit = intval($group['timelimit']); // 获取用户组的截止日期
    $timeadd = 0;
    if ($timelimit > 0) {
        $timeadd = strtotime($timelimit . ' days');
    }
    // 如果是副创始人，则比较副创始人组和该副创始人用户的有效时间
    ...
    if (user_is_vice_founder()) { // 如果是副创始人，则拥有者id为自己
    ...
    if (!empty($user_have_accounts)) {
        foreach ($user_have_accounts as $account) {
            if (!empty($account['key']) && !empty($account['secret'])) {
                if (in_array($account['level'], array(ACCOUNT_SERVICE_VERIFY))) { // oauth授权必须是认证服务号
                    $oauth_accounts[$account['acid']] = $account['name'];
                }
                // jsoauth授权可是是认证订阅号或者认证服务号
