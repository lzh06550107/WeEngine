...
function wxapp_account_create($account)
{
    ...
    if (empty($_W['isfounder'])) {
        ...
        uni_user_account_role($uniacid, $_W['uid'], ACCOUNT_MANAGE_NAME_OWNER); // 给统一帐号添加拥有者角色
    ...
    if (user_is_vice_founder()) { // 给统一帐号添加副创始人角色
...
{
    $uni_modules = uni_modules(); // 获取统一帐号具有模块
    $wxapp_modules = array();
...
function wxapp_fetch($uniacid, $version_id = '')
{
    ...
    if (empty($version_id)) {
        ...
        if (!empty($_GPC['__wxappversionids'])) { // 从cookie中获取版本id
        ...
        if (empty($wxapp_version_info)) { // 否则获取最新记录
...
function wxapp_update_daily_visittrend()
{
    ...
    $wxapp_stat = $account_api->getDailyVisitTrend(); // 获取小程序访问分析数据
