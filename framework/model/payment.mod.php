...
define('ALIPAY_GATEWAY', 'https://mapi.alipay.com/gateway.do'); // 已经过时
...
function alipay_build($params, $alipay = array())
{
    ...
    if ($params['service'] == 'create_direct_pay_by_user') {
        ...
        $set['app_pay'] = 'Y'; // app_pay=Y：尝试唤起支付宝客户端进行支付，若用户未安装支付宝，则继续使用wap收银台进行支付。
    }
    $prepares = array();
...
function wechat_proxy_build($params, $wechat)
{
    ...
    if (intval($wechat['switch']) == '2') { // 借用支付
        $_W['uniacid'] = $uniacid;
...
function payment_proxy_pay_account()
{
    ...
    if ($setting['payment']['wechat']['switch'] == PAYMENT_WECHAT_TYPE_SERVICE) { // 服务商支付
        $uniacid = intval($setting['payment']['wechat']['service']);
    } elseif ($setting['payment']['wechat']['switch'] == PAYMENT_WECHAT_TYPE_BORROW) { // 借用支付
