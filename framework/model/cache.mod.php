...
{
    load()->func('file');
    rmdirs(IA_ROOT . '/data/tpl', true); // 删除所有模板文件
}

/**
 * 更新设置项缓存，从设置表中获取最新数据，然后写入缓存中
 */
...
/**
 * 重建系统可使用的模块缓存，即删除禁止的模块缓存
 */
function cache_build_module_status()
{
    load()->model('cloud');
    $cloud_modules = cloud_m_query(); // 查询本地模块的最新信息
    $module_ban = is_array($cloud_modules['pirate_apps']) ? $cloud_modules['pirate_apps'] : array(); // 远程禁止模块
    $local_module = setting_load('module_ban'); // 加载本地禁止模块
    // 合并本地和远程禁止模块
    ...
    if (!empty($update_modules)) {
        foreach ($update_modules as $module) {
            cache_build_module_info($module); // 删除指定模块和设置的缓存信息
        }
    }
    setting_save($module_ban, 'module_ban'); // 缓存远程禁止模块到设置表中
...
function cache_build_frame_menu()
{
    ...
    if (!empty($system_menu) && is_array($system_menu)) {
        ...
        foreach ($system_menu as $menu_name => $menu) { // 1、处理顶层菜单项
            $system_menu[$menu_name]['is_system'] = true; // 表示为系统内置菜单
            // 如果数据库中存在该菜单是否显示，则使用
            $system_menu[$menu_name]['is_display'] = empty($system_menu_db[$menu_name]) || !empty($system_menu_db[$menu_name]['is_display']) ? true : false;
            // 如果数据库中存在该菜单的排序，则使用，否则按遍历顺序排序
            ...
            foreach ($menu['section'] as $section_name => $section) {// 2、处理区域二级菜单项
                $displayorder = max(count($section['menu']), 1); // 获取当前分区中菜单项个数

                if (empty($section['menu'])) {
                    $section['menu'] = array();
                }
                // 如果该分区中存在自定义的二级菜单项，则添加到区域
                ...
                if (!empty($add_menu)) {
                    foreach ($add_menu as $permission_name => $menu) {
                        ...
                        $section['menu'][$permission_name] = $menu; // 增加自定义二级菜单到区域中
                    }
                }
                $section_hidden_menu_count = 0;
                foreach ($section['menu'] as $permission_name => $sub_menu) { // 3、处理区域中每一个二级菜单项
                    $sub_menu_db = $system_menu_db[$sub_menu['permission_name']]; // 菜单表中是否存在该二级菜单项
                    ...
                    if (empty($system_menu[$menu_name]['section'][$section_name]['menu'][$permission_name]['is_display'])) {// 如果该菜单项最后计算还是不显示，则统计隐藏菜单项数
                        $section_hidden_menu_count++;
                    }
                }
                // 如果区域要显示，但菜单项都隐藏了，则该区域也隐藏
...
function cache_build_uninstalled_module()
{
    ...
    if (!empty($cloud_module) && !is_error($cloud_module)) {
        foreach ($cloud_module as $module) {
            ...
            if (!empty($installed_module[$module['name']]) && ($installed_module[$module['name']]['app_support'] != $app_support || $installed_module[$module['name']]['wxapp_support'] != $wxapp_support)) {
                $upgrade_support_module = true; // 模块更新？？
            }
    ...
    if (is_array($module_file) && !empty($module_file)) {
        foreach ($module_file as $modulepath) {
            ...
            // 更新模块支持
            if (!empty($installed_module[$modulepath]) && ($manifest['app_support'] != $installed_module[$modulepath]['app_support'] || $manifest['wxapp_support'] != $installed_module[$modulepath]['wxapp_support'] || $manifest['webapp_support'] != $installed_module[$modulepath]['webapp_support']) ) {
            ...
            if (!in_array($manifest['name'], array_keys($installed_module)) || $upgrade_support_module) {
                // TODO by lzh 获取模块图标
                $iconUrl = '';
                if(file_exists($path.$modulepath.'/icon.jpg')) {
                    $iconUrl = $manifest['name'] . '/icon.jpg';
                }else if(file_exists($path.$modulepath.'/preview.jpg')){
                    $iconUrl = $manifest['name'] . '/preview.jpg';
                }
                ...
                if ($upgrade_support_module) { // 如果是模块更新支持类型，则
                    ...
                    if ($module_info['phoneapp_support'] == 2 && $installed_module[$module_info['name']]['phoneapp_support'] != 2) {
                    ...
                    if ($module_info['phoneapp_support'] == 2) {
