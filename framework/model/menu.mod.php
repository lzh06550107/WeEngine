...
function menu_update_currentself() {
	...
	$default_menu = $default_menu_info['selfmenu_info']; // 菜单信息
...
function menu_update_conditional() {
	...
	if (!empty($conditional_menu_info['conditionalmenu'])) { // conditionalmenu为个性化菜单列表
		foreach ($conditional_menu_info['conditionalmenu'] as $menu) {
			$data = array(
				...
				'client_platform_type' => $menu['matchrule']['client_platform_type'], // 客户端版本
				...
				'menuid' => $menu['menuid'], // 个性化菜单id
				'status' => STATUS_ON,
			);
			if (!empty($menu['matchrule'])) { // 菜单匹配规则
