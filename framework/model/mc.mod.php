...
/**
 * 如果是认证号，则静默获取用户信息, 不需要跳转到网页授权获取用户信息；
 * 否则，通过网页授权获取用户信息。
 *
 * @param string $url
 * @return array
...
/**
 * 获取公众号粉丝标签列表，并同时更新标签组表mc_fans_groups
 * @return array
 */
function mc_fans_groups($force_update = false)
{
    ...
    if (!empty($tags['tags'])) {
        ...
        foreach ($tags['tags'] as $da) {
            if ($da['id'] == 1) { // 自定义标签id从100开始，1为腾讯定义的标签
                continue;
            }
            $tags_tmp[$da['id']] = $da;
        }
...
/**
 * 更新会员所在的会员组。积分变动会导致用户升级，从而会员组会变化。
 * */
 ...
 * @param string $url
 * @param string $remark
 * @return array|bool|error|string
 */
...
function mc_init_fans_info($openid, $force_init_member = false)
{
    ...
    if (is_array($openid)) {
        $fans_list = $account_api->fansBatchQueryInfo($openid); // 批量获取指定的粉丝信息
    } else {
        $fans_list = $account_api->fansQueryInfo($openid); // 根据用户openid获取用户信息
    }
    ...
    foreach ($fans_list as $fans) {
        // 没有关注的粉丝，可能是曾经关注，现在不关注的用户，发生在用户取消关注时，网络异常导致应用没有及时更新
        ...
        // 清空公众号运营者对粉丝的备注、用户关注的渠道来源、二维码扫码场景、二维码扫码场景描述
        ...
        if ($force_init_member) {
            ...
            if (empty($fans_mapping['uid'])) { // 如果用户还不是应用会员，则自动注册
        ...
        if (!empty($fans_mapping)) { // 更新粉丝和会员映射信息
